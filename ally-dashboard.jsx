...
// ════════════════════════════════════════════════════════════════════════════
//  ALLY DASHBOARD — matches ally-final.jsx exactly
//  Reads the same window.storage key "aidx". Columns match the bot 1:1.
// ════════════════════════════════════════════════════════════════════════════
...
export default function Dashboard() {
  ...
  async function load() {
    ...
    try {
      let idx = [];
      try { const r = await window.storage.get("aidx", true); if (r) idx = JSON.parse(r.value); } catch {}
      const items = [];
      for (const id of idx) { try { const r = await window.storage.get(id, true); if (r) items.push(JSON.parse(r.value)); } catch {} }
      setResps(items.sort((a,b) => new Date(b.ts) - new Date(a.ts)));
    } catch {}
    setLoading(false);
  }
  ...
  // ── COMPUTED (mapped to the brief, using only fields the bot collects) ──
              ...
              <div style={{width:sel?280:"100%",flexShrink:0,overflowY:"auto",maxHeight:"70vh"}}>
                {filtered.map(r=>(
                  <div key={r.id} onClick={()=>setSel(sel?.id===r.id?null:r)}
                    style={{padding:"11px 13px",marginBottom:7,background:sel?.id===r.id?"rgba(191,160,98,.08)":"rgba(242,237,230,.03)",border:"1px solid "+(sel?.id===r.id?"rgba(191,160,98,.4)":"rgba(242,237,230,.08)"),borderRadius:11,cursor:"pointer",transition:"all .15s"}}>
                ...
                <div style={{flex:1,overflowY:"auto",maxHeight:"70vh"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:"rgba(242,237,230,.3)"}}>{new Date(sel.ts).toLocaleString()} · {sel.lang==="es"?"Spanish":"English"} · LP {sel.lp} · {ARC_LABELS[sel.arc]}</div>
                    </div>
                  ...
                  <div style={{background:"rgba(242,237,230,.03)",border:"1px solid rgba(242,237,230,.08)",borderRadius:12,overflow:"hidden",marginBottom:14}}>
                    {[
                      ["Name","name",""],["DOB","dob",""],["City","city","Block A"],["Occupation","occ","Block A"],
